...
from numpy import (pi,exp,logspace,sum,argmin,array,newaxis,repeat,copy,
                   arange,empty_like,atleast_1d,isscalar,set_printoptions)
from matplotlib.pyplot import figure, show
from findnearest import find_nearest



def fluxgen(E,E0,Q0,Wbc,Bm,Bhf,bl,bm,bh):
    E0 = atleast_1d(E0)
    Wb=Wbc*E0

    isimE0 = find_nearest(E,E0)[0]

    E = repeat(E[:,newaxis], E0.size, axis=1) #identical columns

    base = gaussflux(E,E0,Q0,Wb)
    arc = copy(base)

    low = letail(E,E0,Q0,bl)
    arc += low #intermediate result

    mid = midtail(Bm,E,E0,arc,isimE0,bm)
    arc += mid #intermediate result

    hi = hitail(Bhf,E,E0,arc,isimE0,bh)
    arc += hi

    diprat(E,E0,arc,isimE0)
...
def midtail(Bm,E,E0,arc,isimE0,bm):
...
def writeh5(h5fn,arc,E,E0,Q0,Wbc,Bm,Bhf,bl,bm,bh):
    import h5py

    with h5py.File(h5fn, libver='latest') as fid:
        ...
        fid.create_dataset('/params/Bm',data=Bm)
        fid.create_dataset('/params/Bhf',data=Bhf)
...
if __name__ == '__main__':
    ...
    p.add_argument('-s','--save',help='write output to HDF5',type=str,default=None)
    args = p.parse_args()
    ...
    useE0 = E0>args.E0min
    E0 = E0[useE0]
    Q0 = array([8e10, 3e11, 1.0e12, 2.8e12, 5.0e12]); Q0 = Q0[useE0]
    Wbc = array([1, 0.75, 0.5, 0.375, 0.25]); Wbc = Wbc[useE0]

    #bl,bm,bh set the exponential slope
    #bl = array([2., 1.5, 1.4, 1.2, 1.]); bl = bl[useE0]
    bl = repeat(0.8,useE0.size); bl = bl[useE0]
    bm = array([3., 3.,  3.,   2.5,  2.]); bm = bm[useE0]
    bh = array([4., 4.,  4.,   3.5,  2.5]); bh = bh[useE0]


    Bm = array([2e3, 5e3, 6e3, 1e4, 1.8e4]); Bm = Bm[useE0]
    Bhf = array([0.145, 0.15, 0.145, 0.2, 0.35]); Bhf = Bhf[useE0]

    #E0 = array([2500., 5000., 7500., 10000.])
    #Q0 = array([8.0e11, 8.0e11, 8.0e11, 8.0e11])

    arc,low,mid,hi,base = fluxgen(E,E0,Q0,Wbc,Bm,Bhf,bl,bm,bh)

    plotflux(E,E0,hi,low,mid,arc,args.debug)
    if args.save is not None:
        writeh5(args.save,arc,E,E0,Q0,Wbc,Bm,Bhf,bl,bm,bh)